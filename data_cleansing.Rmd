...
model <- lm(target_deathrate ~ percentmarried * incidencerate  + pcths25_over +
              povertypercent + pctbachdeg25_over + pctemployed16_over + medincome * pcths18_24 + ctbachdeg18_24 * pcths25_over
                   , data = df_cleaned)
...
# Linear model: target_deathrate ~ f.incidencerate --> 1.5%
lm_incidencerate <- lm(target_deathrate ~ f.incidencerate, data = df_cleaned)
summary(lm_incidencerate)

# Linear model: target_deathrate ~ f.medincome --> 1.9%
...
#Interaction models 
```{r}
# Fit a linear model with all relevant predictors
lm_full <- lm(target_deathrate ~ f.percentmarried + f.incidencerate + f.medincome + f.pcths18_24, data = df_cleaned)
...
lm_interaction <- lm(target_deathrate ~ percentmarried * incidencerate + medincome * pcths18_24 + studypercap + pctbachdeg18_24 * pcths25_over, data = df_cleaned)
