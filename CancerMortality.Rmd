...
df_cleaned$pcths <- (df$pcths18_24 + df$pcths25_over)

# pctbachdeg18_24 with pctbachdeg25_over
df_cleaned$pctbach <- (df$pctbachdeg18_24 + df$pctbachdeg25_over)
...
library(lmtest)
library(sandwich)
m1 <- lm(target_deathrate ~ medincome, data = df_cleaned[,c(1:21, 54:55)])
summary(m1)
par(mar = c(5, 5, 4, 2)) 
...
m3 <- lm(target_deathrate ~ medincome + incidencerate + pcths, data= df_cleaned[,c(1:21, 54:55)])
summary(m3)
